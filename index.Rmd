...
Our main question is how has HIV prevalence changed overtime in various sub-populations of New York City throughout 2011-2015? This question is
...
overtime in various sub-populations of New York City, there are numerous other questions
...
* How has HIV prevalence changed overtime in various regions and sub-populations of New York City throughout 2011-2015?
...
* Based on the analysis of the provided data, we found that
there was an average of `r summary_info$mean_HIV_diagnoses` HIV diagnoses annually. The
mean HIV diagnosis rate was `r summary_info$mean_HIV_diagnosis_rate` per 100,000 population,
indicating a significant prevalence of the disease. Over
the course of the data collection period, there were a total
of `r summary_info$total_deaths` deaths reported. The impact of the disease seems to
be more severe among males, with `r summary_info$total_males_affected_HIV` males affected by HIV,
compared to `r summary_info$total_females_affected_HIV` females. This data underscores the importance of gender-sensitive approaches to HIV prevention and treatment
strategies. The information provided by this data provides valuable
insights into the HIV/AIDS situation in the area and can guide future
...
* We include the table below to provide a summarized view of 
the HIV situation across different boroughs in the dataset. 
This aggregate view allows us to quickly compare and contrast 
the impact of HIV among different boroughs, as well as between
different genders within those boroughs.


```{r table_aggregate, echo=FALSE}
# Run the script to generate the table
source("aggregate_table.R")

# Load the table
df_aggregate <- readRDS("df_aggregate.RDS")

# Display table
knitr::kable(df_aggregate, caption = "Aggregate HIV data by Borough.")

```

### Chart 1: HIV Diagnosis by Gender from 2011 to 2015
```{r chart_first, echo = FALSE, message = FALSE}
source("first_chart.R")
plot(chart_first)
```

This chart is a visual representation depicting the number of HIV diagnoses by gender from 2011 to 2015 in the state of New York. 

#### Purpose

The bar chart illustrates the distribution of HIV diagnoses among different genders. Each gender is represented by a bar, and its height reflects the respective number of HIV diagnoses. This visual aid allows us to easily compare HIV diagnosis patterns across genders from 2011 to 2015. Examining these patterns allow for analysis of gender-specific disparities in HIV transmission, providing valuable and more targeted insights into public health protocols.

#### Insights
-In addition to the gender-specific distribution of HIV diagnoses, the analysis also highlights the substantial disparities among different gender categories. The chart underscores that **males** experienced the **highest** number of HIV diagnoses during the five-year period, surpassing 2,500 cases. Conversely, **females** had the **second-highest** count, with just under 1,000 cases, and the **transgender** category displayed the **lowest** number of cases. The discrepancy in **transgender** diagnoses may reflect a **smaller population undergoing HIV testing** within the category, leading to a reduced overall count of positive cases. This data suggests potential variations in **risk behaviors, healthcare access, and prevention efforts**, contributing to this variability.

-The "ALL" category from the gender variable was excluded when plotting the bar chart. By omitting this category, which aggregates diagnoses across all boroughs in the state of New York, the focus remains on the distinct gender-specific trends totaled from boroughs.

### Chart 2: HIV Diagnoses Across Different Race between 2011 and 2015 in NYC
...
This chart visualizes the HIV diagnoses across different racial groups from 2011
to 2015 in NYC.

#### Purpose

By comparing the HIV diagnoses across different racial groups, this chart seeks
to express which group is the most likely to contract HIV. The chart allows us
to understand which group needs more attention and resources in reducing the
risk of contracting HIV and allows us to explore the underlying systems/factors
that cause the group to have the highest HIV rate.

#### Insights

- **Black** people have the **highest** number of HIV diagnoses recorded from
2011-2015, while **Asian/Pacific Islander** people have the **lowest**.
  - There's a big gap in HIV diagnoses between these two groups, while there's
  a smaller gap between Black and Latino/Hispanic. And HIV diagnoses for
  White is approximately half that of Latino/Hispanic.
- This result may indicate the **lack of access to healthcare and proper HIV**
**education** in Black and Latino/Hispanic communities. 
  - We need to make preventive programs and treatment more accessible to these
  communities.

### Chart 3: HIV and AIDS Related Death Rate Overtime (2011-2015) in New York
```{r chart_third, echo = FALSE, message = FALSE}
source("third_chart.R")
plot(chart_third)
```

This chart illustrates the trend of HIV and AIDS related death rate over time from the years of 2011-2015 in New York. Each point represents the HIV and AIDS related death rate for the designated year and showcases the correlation between these two variables.

#### Purpose
By graphing the correlation between HIV and AIDS related death rate and year, this chart allows us to observe the trend of how HIV and AIDS has affected mortality of those who have the disease over the years of 2011-2015. This chart allows us to analyze whether the HIV/AIDS epidemic has been increasing, decreasing, or stabilizing overtime in New York. Furthermore, the illustrated trend in the chart can be used to provide insight into public health interventions, campaigns, and overall HIV/AIDS treatment effectiveness in New York from 2011-2015.

#### Insights
- There is a significant **downward trend** in the HIV and AIDS related death rate from 2011-2015
  - Overall, the death rate decreases from **13.6 in 2011** to **9.6 in 2015**
  - The magnitude of change in the death rate is notable at 4.0 points
- The downward trend suggests **improvement in sexual & health education and treatment efficiency** targeting HIV and AIDS mortality
  - This trend may be influenced by the increase of awareness campaigns, testing access, and treatment support from NYC health in New York throughout 2011-2015.
